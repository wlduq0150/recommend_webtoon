...
import { genresToText } from "./functions/genresToText.function";
...
export class RecommendService {
    ...
    async createRecommendGenre(webtoonId: string): Promise<string[]> {
        ...
        genreCounterArray.sort((a: [string, number], b: [string, number]) => b[1] - a[1]);
    ...
    async initWebtoonDescriptionEMB(initRecommendGenreOptionDto: InitRecommendGenreOptionDto) {
        ...
        for (let webtoon of webtoons) {
            ...
            const embVector = JSON.stringify(await this.openaiService.createEmbedding(description));
    ...
    async createRecommendWebtoon(
        ...
        // 모든 장르에 대해 장르의 뜻을 문자열로 합치기
        const genreText = await genresToText(genres, this.genreService);
        ...
        for (let webtoon of webtoons) {
            if (webtoon.embVector) {
                similarityComapre[webtoon.id] = this.openaiService.calcSimilarityFromEmbedding(
        ...
        webtoons.sort((a, b) => {
            return similarityComapre[a.id] - similarityComapre[b.id];
        });
    ...
    async recommendWebtoon(recommendWebtoonDto: RecommendWebtoonDto): Promise<String[]> {
        const { userId, genres, category, episodeLength, newExcludeWebtoonIds } =
            recommendWebtoonDto;
        // cache-key
        ...
        const excludeWebtoonIdsCache: string =
            await this.cacheManager.get(excludeWebtoonIdsCacheKey);
        ...
        // 캐싱
        this.cacheManager.set(excludeWebtoonIdsCacheKey, JSON.stringify(excludeWebtoonIds), 120000);
